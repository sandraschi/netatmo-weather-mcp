...
class PredictiveAnalyticsTools:
    ...
    async def generate_predictions(
        self, prediction_type: str, station_id: str, forecast_hours: int, confidence_threshold: float
    ) -> dict[str, Any]:
        """
        ...
        try:
            ...
            if prediction_type == "short_term":
                ...
                result = await self._generate_trend_analysis(client, station_id, forecast_hours)
            elif prediction_type == "anomaly_detection":
                result = await self._generate_anomaly_predictions(client, station_id, forecast_hours)
            else:
                raise ValueError(f"Unsupported prediction type: {prediction_type}")

            # Add automation suggestions based on predictions
            automation_suggestions = await self._generate_automation_suggestions(result, prediction_type)

            # Add alert conditions
            alert_conditions = await self._generate_alert_conditions(result, prediction_type, confidence_threshold)
            ...
            logger.error(
                "Prediction generation failed", prediction_type=prediction_type, station_id=station_id, error=str(e)
            )
            return {
                "success": False,
                "prediction_type": prediction_type,
    ...
    async def _generate_short_term_predictions(
        self, client: NetatmoClient, station_id: str, forecast_hours: int, confidence_threshold: float
    ) -> dict[str, Any]:
        ...
        if len(historical_data) < 6:  # Need at least 6 hours of data
            return {"predictions": [], "average_confidence": 0.0, "model_used": "insufficient_data"}
    ...
    async def _generate_trend_analysis(
        self, client: NetatmoClient, station_id: str, analysis_hours: int
    ) -> dict[str, Any]:
    ...
    async def _generate_anomaly_predictions(
        self, client: NetatmoClient, station_id: str, detection_hours: int
    ) -> dict[str, Any]:
        """Generate anomaly detection predictions."""
        # Use sampling manager for anomaly detection
        result = await self._sampling_manager.perform_anomaly_detection_sampling(client, station_id, iterations=3)
    ...
    async def _generate_automation_suggestions(
        self, prediction_result: dict[str, Any], prediction_type: str
    ) -> list[dict[str, Any]]:
    ...
    async def _generate_alert_conditions(
        self, prediction_result: dict[str, Any], prediction_type: str, confidence_threshold: float
    ) -> list[dict[str, Any]]:
        ...
        if prediction_type == "anomaly_detection":
            ...
            high_confidence_anomalies = [
                p for p in predictions if p.get("confidence", 0) >= confidence_threshold and p.get("severity") == "high"
            ]
            ...
            extreme_temps = [
                ...
                and (p.get("temperature", 20) > 35 or p.get("temperature", 20) < 0)
            ]
