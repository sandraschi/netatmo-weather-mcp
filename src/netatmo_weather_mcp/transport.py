...
Environment Variables:
    ...
    MCP_PORT: Port for HTTP/SSE. Default: 10823 (fleet 10700+; set MCP_PORT to override)
...
ENV_PORT = "MCP_PORT"  # default: 10823
...
def get_transport_config() -> dict:
    ...
    return {
        ...
        "port": int(os.getenv(ENV_PORT, "10823")),
...
Environment Variables:
  ...
  {ENV_PORT}         Port number (default: 10823)
...
Examples:
  ...
  # HTTP mode (web apps)
  python -m {server_name.replace("-", "_")} --http --port 10823

  # Via environment
  MCP_TRANSPORT=http MCP_PORT=10823 python -m {server_name.replace("-", "_")}
""",
    )
